...
export async function GET(request: Request) {
  ...
  zip.addFile("summary.csv", Buffer.from(csvFile(
    ["workspace", "generated_at", "range_from", "range_to", "validation_runs", "telegram_campaigns", "account_operations", "ai_campaigns", "total_runs", "subscription_expires_at"],
    [[account.email, generatedAt, range.fromValue, range.toValue, validation.length, campaigns.length, accountBatches.length, aiCampaigns.length, allRunCount, account.accessExpiresAt]],
  )));
  ...
  zip.addFile("telegram_campaigns.csv", Buffer.from(csvFile(
    ["campaign_id", "name", "status", "target_type", "mode", "message", "parse_mode", "targets", "processed", "sent", "failed", "skipped", "replied", "sessions", "reply_tracking", "created_at", "started_at", "finished_at", "error"],
    campaigns.map((run) => [run.id, run.name, run.status, run.targetType, run.mode, run.message, run.parseMode, run.totalCount, run.processedCount, run.sentCount, run.failedCount, run.skippedCount, run.repliedCount, run.sessionCount, run.replyTrackingStatus, run.createdAt.toISOString(), run.startedAt?.toISOString(), run.finishedAt?.toISOString(), run.errorMessage]),
  )));
  ...
  zip.addFile("ai_campaigns.csv", Buffer.from(csvFile(
    ["campaign_id", "name", "provider", "status", "duration", "model_id", "preset_id", "sessions", "incoming", "sent", "failed", "created_at", "started_at", "ends_at", "stopped_at", "last_error"],
    aiCampaigns.map((run) => [run.id, run.name, run.provider, run.status, run.durationMode, run.modelId, run.presetId, run.sessions.length, run.messagesReceived, run.messagesSent, run.failedCount, run.createdAt.toISOString(), run.startedAt.toISOString(), run.endsAt?.toISOString(), run.stoppedAt?.toISOString(), run.lastError]),
  )));
