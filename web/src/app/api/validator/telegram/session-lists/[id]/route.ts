...
export async function PUT(request: Request, { params }: Context) {
  ...
  if (!exists) return NextResponse.json({ error: "Session fleet not found" }, { status: 404 });
  ...
  return list ? NextResponse.json({ list }) : NextResponse.json({ error: "A fleet with this name already exists" }, { status: 409 });
}
...
export async function DELETE(_request: Request, { params }: Context) {
  ...
  return deleted.count ? NextResponse.json({ ok: true }) : NextResponse.json({ error: "Session fleet not found" }, { status: 404 });
}
