  ...
  try {
    ...
    if (!parsed.success)
      return NextResponse.json(
        {
          error: parsed.error.issues[0]?.message || "Enter valid login details",
        },
        { status: 400 },
      );
    ...
    if (!credential)
      throw new TelegramControlError(
        "Add your Telegram api_id and api_hash before logging in",
        409,
    ...
    const pendingCount = await prisma.telegramLoginFlow.count({
      where: {
    ...
    if (
      ...
      sessionCount + pendingCount >= account.sessionLimit
    ) {
      throw new TelegramControlError(
        `Your plan allows ${account.sessionLimit} Telegram sessions`,
        403,
        "TELEGRAM_SESSION_LIMIT_EXCEEDED",
      );
    }
    const flow = await runChargedValidatorTask(
      {
        accountId: account.id,
        accessKeyId: account.accessKeyId,
        taskCode: "telegram_login",
        sessions: 1,
        description: "Connect a Telegram account",
      },
      () =>
        prisma.telegramLoginFlow.create({
          data: {
            accountId: account.id,
            accessKeyId: account.accessKeyId,
            ...
            proxyEncrypted: parsed.data.proxyUrl
              ? encryptTelegramData(parsed.data.proxyUrl)
              : null,
