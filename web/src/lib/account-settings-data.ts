...
import { ACCOUNT_SETTINGS_AVATAR_ROOT } from "@/lib/account-settings";
...
export const RANDOM_FIRST_NAMES = [
  ...
  "Adrian", "Bailey", "Dana", "Frances", "Gray", "Holland",
];
...
export async function accountSettingsAvatars() {
  const files = await readdir(ACCOUNT_SETTINGS_AVATAR_ROOT).catch(() => []);
  return files
    .filter((fileName) => /^avatar[\w-]+\.(?:jpe?g|png|webp)$/i.test(fileName))
    .sort((a, b) => a.localeCompare(b, undefined, { numeric: true, sensitivity: "base" }))
    .map((fileName) => ({
      id: path.parse(fileName).name,
      fileName,
      url: `/account-settings/avatars/${encodeURIComponent(fileName)}`,
    }));
}

export function uniqueProfileUsername(base: string, used: Set<string>) {
  const clean = base.replace(/^@/, "").trim().slice(0, 32);
  if (!clean) return "";
  ...
  for (let attempt = 0; attempt < 12; attempt++) {
    const suffix = `_${randomBytes(3).toString("hex").slice(0, 4)}`;
    const candidate = `${clean.slice(0, 32 - suffix.length)}${suffix}`;
    if (!used.has(candidate.toLowerCase())) {
      used.add(candidate.toLowerCase());
      return candidate;
    }
  }
  return `${clean.slice(0, 25)}_${Date.now().toString(36).slice(-6)}`.slice(0, 32);
}
